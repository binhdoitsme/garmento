...
def draw_pose(
    ...
    model_type: Literal["body_25", "coco"],
):
    ...
    for pose in poses:
        for limb, color in zip(limbSeq, colors):
            ...
            cv2.fillConvexPoly(cur_canvas, np.array(polygon), color)
...
def padRightDownCorner(img, stride, padValue):
    ...
    pad = [
        0,  # up
        0,  # left
        0 if (h % stride == 0) else stride - (h % stride),  # down
        0 if (w % stride == 0) else stride - (w % stride),  # right
    ]
