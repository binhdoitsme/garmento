...
from . import util
from .model import Body25PoseModel, CocoPoseModel
...
class OpenPose(object):
    def __init__(self, model_type: Literal["body_25", "coco"]):
        if model_type == "body_25":
            self.model: torch.nn.Module = Body25PoseModel()
            self.njoint: int = 26
            self.npaf: int = 52
            self.model.load_state_dict(torch.load(model_body25))
        else:
            self.model = CocoPoseModel()
            self.njoint = 19
            self.npaf = 38
    ...
    def __call__(self, original_img: npt.NDArray[np.uint8]):
        ...
        heatmap_avg = np.zeros((original_img.shape[0], original_img.shape[1], self.njoint))
        paf_avg = np.zeros((original_img.shape[0], original_img.shape[1], self.npaf))
        ...
        for m in range(len(multiplier)):
            ...
            padded_img, pad = util.padRightDownCorner(
                transformed_img, stride, padValue
            )
            im = (
                np.transpose(
                    np.float32(padded_img[:, :, :, np.newaxis]), (3, 2, 0, 1)
                )
                / 256
                - 0.5
            )
            ...
            paf = cv2.resize(
                paf, (original_img.shape[1], original_img.shape[0]), interpolation=cv2.INTER_CUBIC
            )

            heatmap_avg += heatmap_avg + heatmap / len(multiplier)  # type: ignore
            paf_avg += +paf / len(multiplier)  # type: ignore
        ...
        for k in range(len(mapIdx)):
            ...
            if nA != 0 and nB != 0:
                ...
                for i in range(nA):
                    for j in range(nB):
                        ...
                        score_with_dist_prior = sum(score_midpts) / len(
                            score_midpts
                        ) + min(0.5 * original_img.shape[0] / norm - 1, 0)
                        criterion1 = len(
                ...
                special_k.append(k)
                connection_all.append(np.array([]))

        # last number in each row is the total parts number of that person
        # the second last number in each row is the score of the overall configuration
        subset = -1 * np.ones((0, self.njoint + 1), dtype=np.float32)
