...
class Body25PoseModel(nn.Module):
    def __init__(self):
        super(Body25PoseModel, self).__init__()
    ...
    def forward(self, x):
        ...
        for s in range(4):
            ...
            for v in range(2, 6):
                tout = self._Mconv_forward(
                    tout, self.models["Mconv%d_stage%d_L2" % (v, s)]
                )
            tout = self.models["Mconv6_7_stage%d_L2" % s][0](tout)  # type: ignore
            tout = self.models["Mconv6_7_stage%d_L2" % s][1](tout)  # type: ignore
            outL2 = tout
        ...
        tout = self.models["Mconv6_7_stage0_L1"][0](tout)  # type: ignore
        tout = self.models["Mconv6_7_stage0_L1"][1](tout)  # type: ignore
        outS0L1 = tout
        ...
        tout = self.models["Mconv6_7_stage1_L1"][0](tout)  # type: ignore
        outS1L1 = self.models["Mconv6_7_stage1_L1"][1](tout)  # type: ignore

        return outS1L1, outL2


class CocoPoseModel(nn.Module):
    def __init__(self):
        super(CocoPoseModel, self).__init__()
